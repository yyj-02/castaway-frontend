...
//     "livestream-id": http.post(
//         Uri.parse(
//             "https://us-central1-castaway-819d7.cloudfunctions.net/app/api"),
//         headers: {
                ...
                onPressed: () => setState(() async {
                  if (fabIconNumber == 0) {
                    ...
                    IO.Socket socket = IO.io(add, <String, dynamic>{
                      ...
                      "extraHeaders": {
                        "id-token": profile.myIdToken,
                        "livestream-id":await jsonDecode(socketres.body)['livestreamId'],
                        //hard coded as of now but supposed to get livestream id using the api
                      },
                    });
                    socket.on(
                        "connect_error", (error) => {print(error.toString())});
                    ...
                    recs() async {
                      if (newflag == 1) {
                        ...
                        socket.emit("upload", File(pather).readAsBytesSync());
                      }
                      if(newflag == 4){
                        socket.disconnect();
                        socket.dispose();
